import { WalletProvider } from "@dojima-wallet/wallet-provider";
import { Address, Network, Transaction, solana } from "@dojima-wallet/types";
import { addressToString } from "@dojima-wallet/utils";
import { SolanaNetwork } from "@dojima-wallet/solana-networks";
import { base58 } from "@dojima-wallet/crypto";
...
export default class SolanaWalletProvider extends WalletProvider {
  ...
  async sendTransaction(
    options: solana.SolanaSendOptions
  ): Promise<Transaction> {
    ...
    if (!options.instructions && !options.to) {
      const programId = await this.getMethod("_deploy")(
        this._signer,
        options.bytecode
      );
    ...
    const hash = await this.getMethod("_sendTransaction")(
      transaction,
      accounts
    );
    return {
  ...
  async signMessage(message: string): Promise<string> {
    ...
    const signature = nacl.sign.detached(
      buffer,
      base58.decode(base58.encode(this._signer.secretKey))
    );
